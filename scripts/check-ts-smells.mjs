...
const SMELLS = [
  ...
  },
  {
    ...
    color: c.yellow,
    regex:
      /Record\s*<\s*(?:string|number)\s*,\s*(?:unknown|any)\s*>|\[\s*[a-zA-Z0-9_]+\s*:\s*(?:string|number)\s*\]\s*:\s*(?:any|unknown)/g,
  ...
  },
  {
    ...
    regex:
      /(?:type\s+[A-Za-z0-9_]+\s*=\s*|:\s*)(?:[A-Za-z0-9_<>\[\]'"\s]+\|\s*){3,}[A-Za-z0-9_<>\[\]'"\s]+/g,
...
if (totalIssues > 0) {
  console.log(
    `⚠️  ${c.yellow}扫描完成！发现 ${c.bold}${totalIssues}${c.reset}${c.yellow} 处坏味道，分布在 ${c.bold}${filesWithIssues}${c.reset}${c.yellow} 个文件中。${c.reset}`,
  );
