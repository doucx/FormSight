好的，我已为您整理并准备好针对 `index.html` 与 `README.md` 的更新计划，使其准确反映 FormSight 当前由 4 大视觉与色彩感知训练模块、OKLab 感知色差引擎、正负形拓扑算法及离线数据分析工具集构成的完整架构。

## [WIP] docs: 更新 README 与 index.html 以匹配全模块系统状态

### 用户需求
1. 更新 `index.html` 的网页标题与元信息，从最初单一的“寻星练习”更新为包含色感、正负形等在内的完整 FormSight 系统定义。
2. 全面重构 `README.md`，完整覆盖系统的 4 大核心训练领域（寻星练习、绝对色感、相对色感、正负形感知）、底层色彩与几何算法原理、数据洞察/离线分析工具集、操作快捷键及技术架构。

### 评论
随着 FormSight 从最初的单一几何寻星工具演进为涵盖空间构图、绝对色感识别、OKLab 相对色彩推移矢量、阿尔伯斯视错觉对抗以及正负形留白认知的综合性视觉训练系统，文档和入口元数据必须同步更新，以准确表达项目的产品定位、理论基础和架构设计。

### 目标
1. 更新 `index.html`，设定统一的系统页面标题与描述。
2. 重构 `README.md`，提供系统化、专业化的模块说明、算法原理、离线分析脚本指南与快速上手流程。

### 基本原理
- **完备性**：确保每个训练维度（Star-Hopping、Color Sense、Relative Color、Negative Space）的子模式、交互规则与评估逻辑均有清晰文档化。
- **算法对齐**：详细记录 OKLab 色差计算 ($\Delta E_{\text{OK}}$)、鞋带公式面积占比、双极透视网格及双算子自适应引擎（3U1D 与 Block Master）。
- **文件重写规范**：对文档结构重写幅度大于 30% 的 `README.md` 采用 `write_file` 完整写入。

### 标签
#intent/document #flow/ready #priority/medium #comp/docs #concept/ui #scope/dx #ai/instruct #task/domain/docs #task/object/readme-and-html #task/action/refactor #task/state/begin #task/status/active

---

### Script

#### Acts 1: 更新 index.html 的网页标题与 meta 信息

我们在 `index.html` 中更新系统的正式名称与视知觉强化训练系统的描述。
...
#### Acts 2: 全面更新 README.md

完整重写 `README.md`，系统性介绍 FormSight 的 4 大训练模块、算法原理、离线分析工具与快速上手指南。
...
### 下一步建议
1. **构建与格式验证**：运行 `npm run check` 及 `npm run build` 确保所有文件符合 Biome 规范且顺利完成编译。
2. **预览页面效果**：通过 `npm run preview` 验证网页 title 与整体静态部署是否一致。
