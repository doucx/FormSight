我将为你调整 `WeaknessAnalyticsModal`（弱点与认知分析弹窗），把原先固定在顶部占位的 4 个全局大盘指标（正确率、总击中题数、训练平均耗时、最高能力层阶）整合为专门的「数据总览 (Overview)」标签页。

## [WIP] refactor: 将核心指标概览整合为专门的分析标签页

### 用户需求
将分析弹窗顶部常驻的 4 个核心统计指标（正确率、总击中题数、训练时长/平均耗时、最高 Level）整合为一个专门独立的标签页，提升各个专项分析视图（SAT、难度抗压、各 Pack 图表）的有效显示空间。

### 评论
这是一个优秀的 UX 重构。原先 4 个大卡片常驻在顶部导致弹窗垂直空间被压缩，且切换到各具体专项分析页面时产生视觉冗余。将其收纳为首个「数据总览 (Overview)」专属 Tab 后，不仅使弹窗整体结构更紧凑，也让总览视图有更充裕的空间进行深入的综合数据展示。

### 目标
1. 在 `zh-CN.json` 与 `en-US.json` 中补充总览 Tab 相关的多语言词条。
2. 重构 `WeaknessAnalyticsModal.tsx`：
   - 增加「数据总览 (Overview)」专属标签页。
   - 将 4 项核心数据指标与综合表现诊断归纳在该标签页中。
   - 切换至其他分析 Tab 时保持页面简洁聚焦。

### 基本原理
在 `WeaknessAnalyticsModal` 中引入 Overview 模式作为 Tab 集合的首项。当用户选中总览 Tab 时，完整呈现 4 维核心大指标矩阵及总体认知诊断；当用户切换到专项分析视图（如反应速度 SAT、难度衰减分析、色环/热力图）时，直接展示对应的视知觉 Canvas 图表与诊断结果。

### 标签
#intent/refine #flow/ready #priority/medium #comp/cli #concept/ui #scope/ux #ai/instruct #task/domain/ui #task/object/weakness-analytics-modal #task/action/refactor #task/state/begin
...
~~~~~json
  "analyticsModal": {
    ...
    "analyzing": "正在分析历史答题数据...",
    "noRecords": "暂无【{{title}}】的练习记录，先去完成几轮练习吧！",
    "overviewTabLabel": "数据总览",
    "overviewTitle": "答题数据总览",
    "overviewSubtitle": "综合统计试炼题量、正确率、平均反应时间与最高层阶",
    "overallEvaluation": "总体评估",
    "sampleSize": "样本量: {{count}} 题",
    "needMoreSamples": "当前样本量较少，多完成几轮训练后将自动生成认知诊断。",
    "satTabLabel": "反应速度-正确率 (SAT)",
...
~~~~~json
  "analyticsModal": {
    ...
    "overviewSubtitle": "Overall statistics on trials, accuracy, response time, and peak level",
    "overallEvaluation": "Overall Evaluation",
    "sampleSize": "Sample size: {{count}} trials",
    "needMoreSamples": "Sample size is small. Cognitive diagnosis will be generated as you complete more trials.",
    "satTabLabel": "Speed-Accuracy (SAT)",
...
#### Acts 2: 重构 WeaknessAnalyticsModal 组件

将 4 个指标卡片移入专门的 Overview 标签页中。
...
### 下一步建议
1. 运行测试或本地预览，验证在不同卡片的分析弹窗中切换「数据总览」与其他分析 Tab 的显示效果。
2. 确认无误后可进行 Git 提交。
