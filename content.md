好的，我将为你生成执行这三项改动的完整计划。

## [WIP] refactor: 优化训练计划编排交互、支持新建状态识别与计划库一键重置

### 用户需求
1. 从计划编辑器中移除“快捷套用官方科学预设”区域，消除与计划库的功能冗余。
2. 在计划编辑器中增加新建与覆盖状态识别：当用户正在创建新计划时，底部保存按钮明确显示为“保存为新计划并使用”，编辑已有计划时显示为“保存修改并使用此计划”。
3. 在全局设置中增加“重置所有计划”操作，允许用户一键清除自定义计划并恢复官方默认推荐流，且不影响练习历史与生涯数据。

### 评论
这些改进厘清了计划库管理与单计划编排的边界，消除了用户在编辑计划时对“是覆盖还是新建”的认知困惑，并在全局设置中补全了计划配置层面的安全重置闭环。

### 目标
1. 在 `src/utils/planStorage.ts` 中新增 `resetPlansToDefault()` 函数。
2. 重构 `src/components/plan/PlanEditorModal.tsx`，移除模板套用区域，根据计划是否已入库动态切换状态标签与保存按钮文案。
3. 在 `src/components/GlobalSettingsModal.tsx` 中增加“恢复初始训练计划”设置项与二次确认弹窗。

### 标签
#intent/refine #flow/ready #priority/high #comp/ui #concept/config #scope/ux #ai/instruct #task/domain/plan #task/object/plan-editor-and-reset #task/action/refactor #task/state/continue

---

### Script

#### Acts 1: 计划存储层增加重置所有计划方法

在 `src/utils/planStorage.ts` 中导出 `resetPlansToDefault()` 函数。
...
#### Acts 2: 精简计划编辑器并增加新建与修改状态识别

更新 `src/components/plan/PlanEditorModal.tsx`，移除多余的预设套用区块，根据 `isNewPlan` 动态展示提示徽章与保存按钮文案。
...
#### Acts 3: 在全局设置中加入“恢复初始训练计划”功能

修改 `src/components/GlobalSettingsModal.tsx`，加入计划库独立重置按钮与二次确认保护。
...
### 下一步建议
1. **测试验证**：在本地浏览器中新建一个空白计划，验证其按钮文案是否准确展示为“保存为新计划并使用”；在全局设置中点击“重置计划”，验证是否安全恢复 4 个官方预设。
2. **阶段三推进**：根据后续需求，可着手实现基于 `CardDefinition.tags` 技能标签的向导式智能一键成组计划功能。
