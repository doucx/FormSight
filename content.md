根据你的要求，我们需要将过重的 `PlanEditorModal.tsx` 进行合理拆分，以提高代码的可维护性和模块化程度。

以下是本次重构的计划：我们将把 `PlanEditorModal.tsx` 拆分为三个独立的子组件：
1. `PlanLibraryDrawer.tsx`：负责计划库的切换、新建空白计划、导入导出、收藏和删除管理。
2. `PlanStageList.tsx`：负责已编排阶段序列的展示、排序、批量调题量和删除。
3. `CardPickerPanel.tsx`：负责按领域过滤并挑选可添加的训练模块卡片。
最后，在 `PlanEditorModal.tsx` 中进行顶层状态的组装。

下面是本次计划的详细内容。

## [WIP] refactor(components): 拆分 PlanEditorModal 组件以降低单文件复杂度

### 用户需求
`src/components/plan/PlanEditorModal.tsx` 包含了过多的职责（计划重命名、计划库管理抽屉、阶段列表维护、卡片挑选器等），导致文件过大且维护困难。需要将其拆分为独立的子组件。

### 评论
本次重构将显著提升 FormSight 项目前端代码的结构清晰度与可测试性，符合高内聚低耦合的组件设计原则。

### 目标
1. 创建 `src/components/plan/editor/` 目录。
2. 抽离计划库管理抽屉为 `PlanLibraryDrawer.tsx`。
3. 抽离阶段列表与操作为 `PlanStageList.tsx`。
4. 抽离卡片挑选器为 `CardPickerPanel.tsx`。
5. 重构 `PlanEditorModal.tsx` 作为顶层容器进行组合。

### 基本原理
通过将单一庞大的 Modal 组件拆分为专注特定子功能的子组件，并通过 Props 与回调函数进行状态流转，既保持了原有交互逻辑的完整性，又极大地精简了主文件的代码行数。

### 标签
#intent/refine #flow/ready #priority/high #comp/cli #concept/ui #scope/core #ai/instruct #task/domain/ui #task/object/plan-editor-modal #task/action/refactor #task/state/begin

---

### Script

#### Acts 1: 创建拆分后的子组件与重构主文件

我们将依次创建 `PlanLibraryDrawer.tsx`、`PlanStageList.tsx`、`CardPickerPanel.tsx`，并重构 `PlanEditorModal.tsx`。

~~~~~act
run_command
~~~~~
~~~~~
mkdir -p src/components/plan/editor
touch src/components/plan/editor/PlanLibraryDrawer.tsx
touch src/components/plan/editor/PlanStageList.tsx
touch src/components/plan/editor/CardPickerPanel.tsx
~~~~~
...
我们已经成功将 `PlanEditorModal.tsx` 拆分成了多个高内聚的子组件，代码结构得到了明显改善。下一步建议运行项目构建或测试套件，验证组件拆分后的功能完整性与类型安全性。
