好的。单锚点模式（`star_single`）等采用 `CardManifest` 架构的模块无法正常显示中文提示（显示为 `packs.xxx.instruction` 键名）的原因是：`TrainingShell` 硬编码了旧版 `PackManifest` 的查找路径（`packs.<packId>.cards.<cardId>.instruction`），而一等公民卡片的多语言词典注册在 `cards.<cardId>` 命名空间下。

我将为系统引入统一的 `getCardInstruction` 辅助函数（类似于 `getCardTitle` 与 `getCardDesc`），实现对新旧命名空间的双向自动兼容回退。

以下是修复计划：

## [WIP] fix(core): 引入 `getCardInstruction` 统一多语言查找路径，修复 TrainingShell 指令解析

### 用户需求
修复单锚点等采用 `CardManifest` 架构的训练模块在悬停 `?` 查看玩法要领时，因多语言命名空间不匹配而显示原始 Key 的问题。

### 评论
该改动对齐了 `getCardTitle` 和 `getCardDesc` 的设计模式，统一了新旧架构在国际化查表上的行为，确保所有模块的 `instruction` 均可正确回退解析。

### 目标
1. 在 `src/core/i18n.ts` 中实现并导出 `getCardInstruction` 函数，支持依次检查 `cards.<cardId>.instruction`、`packs.<packId>.cards.<cardId>.instruction`、静态字段及 `desc`。
2. 在 `src/components/training/TrainingShell.tsx` 中引入 `getCardInstruction` 并替换硬编码的 `t(...)` 查询。

### 基本原理
通过多级降级查询（Fallback），自动适配 PackManifest 架构与 CardManifest 架构的国际化词典位置，从根本上解决 Key 泄露问题。

### 标签
#intent/fix #flow/ready #priority/medium #comp/engine #concept/config #scope/core #ai/instruct #task/domain/i18n #task/object/card-instruction #task/action/fix #task/state/begin

---

### Script

#### Acts 1: 在 `src/core/i18n.ts` 中实现 `getCardInstruction`

我们将参照 `getCardTitle` 和 `getCardDesc` 的双向回退逻辑，在 `src/core/i18n.ts` 中增加 `getCardInstruction`。

~~~~~act
patch_file
src/core/i18n.ts
~~~~~
~~~~~python.old
/**
 * 通用：解析卡片标题多语言回退 (优先卡片级词典 -> Pack词典 -> 静态默认值)
 */
export function getCardTitle(
  card: { id: string; packId?: string; title?: string },
  t = i18n.t,
): string {
  // 1. 优先从 Card 命名空间查询
  const cardKey = `cards.${card.id}.title`;
  const cardTitle = t(cardKey);
  if (cardTitle !== cardKey) return cardTitle;

  // 2. 回退从 legacy Pack 命名空间查询
  const packId = card.packId || 'core';
  const legacyPackKey = `packs.${packId}.cards.${card.id}.title`;
  const legacyTitle = t(legacyPackKey);
  if (legacyTitle !== legacyPackKey) return legacyTitle;

  // 3. 最终回退至静态字段或 ID
  return card.title || card.id;
}
~~~~~
~~~~~python.new
/**
 * 通用：解析卡片标题多语言回退 (优先卡片级词典 -> Pack词典 -> 静态默认值)
 */
export function getCardTitle(
  card: { id: string; packId?: string; title?: string },
  t = i18n.t,
): string {
  // 1. 优先从 Card 命名空间查询
  const cardKey = `cards.${card.id}.title`;
  const cardTitle = t(cardKey);
  if (cardTitle !== cardKey) return cardTitle;

  // 2. 回退从 legacy Pack 命名空间查询
  const packId = card.packId || 'core';
  const legacyPackKey = `packs.${packId}.cards.${card.id}.title`;
  const legacyTitle = t(legacyPackKey);
  if (legacyTitle !== legacyPackKey) return legacyTitle;

  // 3. 最终回退至静态字段或 ID
  return card.title || card.id;
}

/**
 * 通用：解析卡片玩法要领多语言回退 (优先卡片级词典 -> Pack词典 -> 静态字段 -> 描述)
 */
export function getCardInstruction(
  card: { id: string; packId?: string; instruction?: string; desc?: string },
  t = i18n.t,
): string {
  // 1. 优先从 Card 命名空间查询
  const cardKey = `cards.${card.id}.instruction`;
  const cardInst = t(cardKey);
  if (cardInst !== cardKey) return cardInst;

  // 2. 回退从 legacy Pack 命名空间查询
  const packId = card.packId || 'core';
  const legacyPackKey = `packs.${packId}.cards.${card.id}.instruction`;
  const legacyInst = t(legacyPackKey);
  if (legacyInst !== legacyPackKey) return legacyInst;

  // 3. 回退至静态字段
  if (card.instruction) return card.instruction;

  // 4. 最终回退至描述
  return getCardDesc(card, t);
}
~~~~~

#### Acts 2: 在 `src/components/training/TrainingShell.tsx` 中使用 `getCardInstruction`

我们将更新 `TrainingShell`，使其导入并调用 `getCardInstruction` 代替硬编码的路径。

~~~~~act
patch_file
src/components/training/TrainingShell.tsx
~~~~~
~~~~~python.old
import { getCardDesc, getCardTitle, useTranslation } from '../../core/i18n';
import type { CardDefinition } from '../../types/card';
import { formatSecondsToTimer } from '../../utils/time';
import { IdlePauseOverlay } from '../common/IdlePauseOverlay';
import type { SessionHistoryItem } from '../modals/SessionSummaryModal';
import { SessionSummaryModal } from '../modals/SessionSummaryModal';
import { Button } from '../ui/button';

export interface TrainingSessionHandle {
  totalTrials: number;
  elapsedSeconds: number;
  isFinished: boolean;
  isIdle: boolean;
  showAnswer: boolean;
  showSummaryModal: boolean;
  sessionHistory: SessionHistoryItem[];
  resumeFromIdle: () => void;
  handleNextQuestion: () => void;
  handleRequestFinish: () => void;
  handleFinishSession: () => void;
  handleRestartSession: () => void;
}

interface TrainingShellProps {
  card: CardDefinition;
  sessionType: 'training' | 'benchmark';
  currentLevel: number;
  isTargeting?: boolean;
  autoNext: boolean;
  session: TrainingSessionHandle;
  showExitButton?: boolean;
  showTimer?: boolean;
  onExit: () => void;
  children: (state: { disabled: boolean; isIdle: boolean }) => ComponentChildren;
}

export function TrainingShell({
  card,
  sessionType,
  currentLevel,
  autoNext,
  session,
  showExitButton = true,
  showTimer = true,
  children,
}: TrainingShellProps) {
  const { t } = useTranslation();
  const cardTitle = getCardTitle(card, t);
  const instruction =
    t(`packs.${card.packId}.cards.${card.id}.instruction`) || card.instruction || '';
  const desc = getCardDesc(card, t);
~~~~~
~~~~~python.new
import { getCardDesc, getCardInstruction, getCardTitle, useTranslation } from '../../core/i18n';
import type { CardDefinition } from '../../types/card';
import { formatSecondsToTimer } from '../../utils/time';
import { IdlePauseOverlay } from '../common/IdlePauseOverlay';
import type { SessionHistoryItem } from '../modals/SessionSummaryModal';
import { SessionSummaryModal } from '../modals/SessionSummaryModal';
import { Button } from '../ui/button';

export interface TrainingSessionHandle {
  totalTrials: number;
  elapsedSeconds: number;
  isFinished: boolean;
  isIdle: boolean;
  showAnswer: boolean;
  showSummaryModal: boolean;
  sessionHistory: SessionHistoryItem[];
  resumeFromIdle: () => void;
  handleNextQuestion: () => void;
  handleRequestFinish: () => void;
  handleFinishSession: () => void;
  handleRestartSession: () => void;
}

interface TrainingShellProps {
  card: CardDefinition;
  sessionType: 'training' | 'benchmark';
  currentLevel: number;
  isTargeting?: boolean;
  autoNext: boolean;
  session: TrainingSessionHandle;
  showExitButton?: boolean;
  showTimer?: boolean;
  onExit: () => void;
  children: (state: { disabled: boolean; isIdle: boolean }) => ComponentChildren;
}

export function TrainingShell({
  card,
  sessionType,
  currentLevel,
  autoNext,
  session,
  showExitButton = true,
  showTimer = true,
  onExit,
}: TrainingShellProps) {
  const { t } = useTranslation();
  const cardTitle = getCardTitle(card, t);
  const instruction = getCardInstruction(card, t);
  const desc = getCardDesc(card, t);
~~~~~

### 下一步建议
验证在单锚点模式（`star_single`）或其他 `CardManifest` 架构卡片下，悬停 `?` 是否能够正确展示对应的玩法要领中文说明。
