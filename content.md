...
1. 在 `RelativeColorCanvasProps` 中补全可选属性 `enableHoverColorPreview?: boolean;`，解决 TS 编译报错；
2. 在 `useEffect` 的 Hook 回调体中引用 `question.id`，解决 Biome linter 的依赖检测规则告警。
...
修改完成，再次运行 `npm run checkf` 与 `npm run build` 均可通过。
